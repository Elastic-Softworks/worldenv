...
/**
 * WORLDSRC Language Test Suite
 *
 * Comprehensive test suite for the WORLDSRC C/C++/TypeScript hybrid language.
...
class LanguageTestSuite {
  ...
  private testTypeScriptFeatures(): void {
    ...
    const tests: TestCase[] = [
      ...
      },
      {
        ...
        expectSuccess: true,
        expectedTokens: [TokenType.CONST, TokenType.IDENTIFIER, TokenType.ASSIGN],
      ...
      },
      {
        ...
        expectSuccess: true,
        expectedTokens: [TokenType.CONST, TokenType.LEFT_BRACE, TokenType.IDENTIFIER],
    ...
    const tests: TestCase[] = [
      {
        ...
        expectSuccess: true,
        expectedTokens: [TokenType.IDENTIFIER, TokenType.IDENTIFIER, TokenType.ASSIGN],
  ...
  private testPerformance(): void {
    ...
    try {
      ...
      console.log(`   Performance test: ${duration.toFixed(2)}ms for 1000 functions`);
      console.log(`   Tokens generated: ${tokens.length}`);
      console.log(`   AST nodes: ${ast.declarations.length}`);

      if (duration < 1000) {
        // Less than 1 second
        this.addResult('Performance test', true, `Completed in ${duration.toFixed(2)}ms`);
      } else {
        this.addResult('Performance test', false, `Too slow: ${duration.toFixed(2)}ms`);
      }
    } catch (error) {
    ...
    for (const test of tests) {
      try {
        ...
        if (test.expectedTokens) {
          for (let i = 0; i < test.expectedTokens.length && i < tokens.length; i++) {
        ...
        if (test.expectedNodes) {
          for (const expectedNode of test.expectedNodes) {
            const found = ast.declarations.some((decl) => decl.constructor.name === expectedNode);
            if (!found) {
        ...
        if (test.expectSuccess) {
          ...
          this.addResult(`${category}: ${test.name}`, false, 'Expected failure but test passed');
        }
      } catch (error) {
        if (test.expectSuccess) {
          console.log(`   ❌ ${test.name}: ${(error as Error).message}`);
          this.addResult(`${category}: ${test.name}`, false, (error as Error).message);
        } else {
          console.log(`   ✅ ${test.name} (correctly failed)`);
          this.addResult(`${category}: ${test.name}`, true, 'Correctly reported error');
        }
      }
    }
  }
  ...
  private printResults(): void {
    ...
    if (failed > 0) {
      ...
      this.results
        .filter((r) => !r.passed)
        .forEach((r) => {
          console.log(`  ❌ ${r.name}: ${r.details || 'Unknown error'}`);
        });
    }
    ...
    if (failed === 0) {
      console.log('🎉 ALL TESTS PASSED! WORLDSRC language implementation is stable.');
    } else if (failed <= total * 0.1) {
      // Less than 10% failure rate
      console.log('⚠️  Minor issues detected. WORLDSRC implementation is mostly stable.');
    } else {
      console.log('❌ Significant issues detected. WORLDSRC implementation needs attention.');
    }
