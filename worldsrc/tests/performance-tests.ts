...
/**
 * WORLDSRC Performance Test Suite
 *
 * Comprehensive performance testing for the WORLDSRC language implementation.
...
class PerformanceTestSuite {
   ...
   */
  private runLexerTest(name: string, code: string, expectedTokens: number): void {
    const startTime = performance.now();
    ...
    try {
      ...
      console.log(
        `   ✅ ${name}: ${duration.toFixed(2)}ms (${tokensPerSecond.toFixed(0)} tokens/sec)`
      );
   ...
   */
  private runParserTest(name: string, code: string, expectedNodes: number): void {
    try {
      const lexer = new Lexer(code);
      const tokens = lexer.tokenize();
      ...
      console.log(
        `   ✅ ${name}: ${duration.toFixed(2)}ms (${nodesPerSecond.toFixed(0)} nodes/sec)`
      );
  ...
  private runFullCompilationTest(name: string, code: string): void {
    ...
    try {
      ...
      console.log(`      Tokens: ${metrics.tokenCount} (${tokensPerSecond.toFixed(0)}/sec)`);
      ...
      console.log(`   ❌ ${name}: Failed (${metrics.totalTime.toFixed(2)}ms)`);
...
`;
  ...
  private printResults(): void {
    ...
    const successfulTests = this.results.filter((r) => r.success && r.duration > 0);
    if (successfulTests.length > 0) {
      ...
      if (tokenTests.length > 0) {
        const avgTokensPerSec =
          tokenTests.reduce((sum, r) => sum + (r.tokensPerSecond || 0), 0) / tokenTests.length;
        console.log(`  Average tokens/sec: ${avgTokensPerSec.toFixed(0)}`);
      }

      const memoryTests = successfulTests.filter((r) => r.memoryUsage);
      if (memoryTests.length > 0) {
        const totalMemory = memoryTests.reduce((sum, r) => sum + (r.memoryUsage || 0), 0);
    ...
    if (failed === 0) {
      console.log('🎉 ALL PERFORMANCE TESTS PASSED! WORLDSRC performance is acceptable.');
    } else if (failed <= total * 0.1) {
      console.log('⚠️  Minor performance issues detected. Review failed tests.');
    } else {
      console.log('❌ Significant performance issues detected. Optimization needed.');
    }
