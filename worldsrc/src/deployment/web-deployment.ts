...
/*
         ...
         web-deployment.ts
           ---
           web deployment implementation for WORLDSRC language.
...
/*
         ...
         WebDeployment
           ...
           compiles WORLDSRC to optimized HTML/CSS/JS bundle.
...
export class WebDeployment extends BaseDeploymentTarget {
  ...
  */
  ...
  public async deploy(): Promise<WebBuildResult> {
    ...
    try {
      ...
      const serviceWorkerFiles = await this.generateServiceWorker(optimizedFiles);
      ...
      return {
        ...
        files: [],
        errors: this.getDiagnostics().filter(d => d.severity === 'error') as any[],
        warnings: this.getDiagnostics().filter(d => d.severity === 'warning') as any[],
        htmlFiles: [],
        cssFiles: [],
        jsFiles: [],
        assetFiles: [],
  ...
  */
  ...
  private async initializeWebBuild(): Promise<void> {
    ...
    if (this.webOptions.browserTargets && this.webOptions.browserTargets.length === 0) {
      this.addWarning('No browser targets specified, using defaults', BuildStage.INITIALIZATION);
    }
    ...
    for (const dep of requiredDeps) {
      try {
        ...
        this.addWarning(`Missing optional dependency: ${dep}`, BuildStage.INITIALIZATION);
      }
    }
    ...
    if (this.webOptions.htmlTemplate) {
      ...
      console.log(`Using custom HTML template: ${this.webOptions.htmlTemplate}`);
    }
  ...
  */
  ...
  private async prepareSourceFiles(): Promise<BuildArtifact[]> {
    ...
    /* scan source directory for WORLDSRC files */
    const worldsrcFiles = await this.scanForFiles(this.options.sourceDirectory, '.ws');
    ...
    const assetFiles = await this.scanForFiles(this.options.sourceDirectory, '.png', '.jpg', '.gif', '.svg', '.ico');
  ...
  /*
           ...
           compileToTypeScript()
             ---
           compiles WORLDSRC source files to TypeScript.

  */

  private async compileToTypeScript(sourceFiles: BuildArtifact[]): Promise<BuildArtifact[]> {
    console.log('🔧 Compiling WORLDSRC to TypeScript...');

    const compiledFiles: BuildArtifact[] = [];

    for (const file of sourceFiles) {
      if (file.metadata?.needsCompilation) {
        /* would use WORLDSRC compiler here */
        const compiledContent = await this.compileWorldSrcFile(file.path);

        const outputPath = file.path.replace('.ws', '.ts');
  ...
  */
  ...
  private async bundleAssets(compiledFiles: BuildArtifact[]): Promise<BuildArtifact[]> {
    ...
    if (jsFiles.length > 0) {
      ...
      if (this.webOptions.optimization?.bundleSplitting !== false) {
  ...
  */
  ...
  private async optimizeForWeb(bundledFiles: BuildArtifact[]): Promise<BuildArtifact[]> {
  ...
  */
  ...
  private async generateHTML(optimizedFiles: BuildArtifact[]): Promise<BuildArtifact[]> {
    ...
    htmlFiles.push({
      ...
      metadata: {
        isMainPage: true,
        includedAssets: [...jsFiles.map(f => f.path), ...cssFiles.map(f => f.path)]
      }
    });
    ...
    console.log(`🏗️ Generated ${htmlFiles.length} HTML files`);
    return htmlFiles;
  }
  ...
  */
  ...
  private async processAssets(): Promise<BuildArtifact[]> {
    ...
    console.log(`🖼️ Processed ${processedAssets.length} assets`);
  ...
  */
  ...
  private async generateServiceWorker(optimizedFiles: BuildArtifact[]): Promise<BuildArtifact[]> {
    ...
    const serviceWorkerContent = this.createServiceWorkerContent(optimizedFiles);
  ...
  */
  ...
  private async finalizeWebBuild(buildFiles: {
    ...
    const jsSize = buildFiles.js.reduce((sum, artifact) => sum + artifact.size, 0);
    const cssSize = buildFiles.css.reduce((sum, artifact) => sum + artifact.size, 0);
    const assetSize = buildFiles.assets.reduce((sum, artifact) => sum + artifact.size, 0);
  ...
  */
  ...
  /*
           ...
           compileWorldSrcFile()
             ---
           compiles a single WORLDSRC file to TypeScript.

  */

  private async compileWorldSrcFile(filePath: string): Promise<string> {
    /* would use actual WORLDSRC compiler */
    return `
// Generated from ${filePath}
export class WorldSrcGame {
  ...
  private async init(): Promise<void> {
    console.log('WORLDSRC game initialized');
  }
...
if (typeof window !== 'undefined') {
  const game = new WorldSrcGame('gameCanvas');
  game.run();
}
`;
  }
  ...
  */
  ...
  private async createJavaScriptBundle(jsFiles: BuildArtifact[]): Promise<BuildArtifact> {
    const bundleContent = `
// WORLDSRC Main Bundle
import { WorldSrcGame } from './worldsrc-game';
...
`;
  ...
  */
  ...
  private async generateIndexHTML(jsFiles: BuildArtifact[], cssFiles: BuildArtifact[]): Promise<string> {
...
<head>
  ...
  <title>${seo.title || 'WORLDSRC Game'}</title>
  <meta name="description" content="${seo.description || 'A game built with WORLDSRC'}">
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Offline - WORLDSRC Game</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      display: flex;
...
<body>
  ...
  */
  ...
  private createServiceWorkerContent(files: BuildArtifact[]): string {
    const cacheableFiles = files.filter(f => f.type !== 'html').map(f => this.getRelativePath(f.path));

    return `
const CACHE_NAME = 'worldsrc-cache-v${this.options.buildNumber || 1}';
...
`;
  ...
  */
  ...
  private createWebManifestContent(): string {
    const manifest = {
      name: 'WORLDSRC Game',
      short_name: 'WorldSrc',
      description: 'A game built with WORLDSRC',
      start_url: '/',
      display: 'standalone',
  ...
  */
  ...
  private async scanForFiles(directory: string, ...extensions: string[]): Promise<string[]> {
  ...
  private getRelativePath(filePath: string): string {
    return filePath.replace(this.options.outputDirectory, '').replace(/^\//, '');
  }
  ...
  protected async validatePlatform(): Promise<{
    ...
    if (this.webOptions.browserTargets) {
      ...
      for (const target of this.webOptions.browserTargets) {
        if (!validTargets.some(valid => target.toLowerCase().includes(valid))) {
          warnings.push({
