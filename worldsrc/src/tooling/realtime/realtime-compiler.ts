...
import { SimpleAnalyzer } from '../../semantic/simple-analyzer';
import { SymbolTable } from '../../semantic/symbol-table';
import { WorldSrcCodeGenerator, CompilationTarget, OptimizationLevel } from '../../codegen';
...
export class RealtimeCompiler extends EventEmitter {
  ...
  private codeGenerator: WorldSrcCodeGenerator;
  ...
  constructor() {
    ...
    this.codeGenerator   = new WorldSrcCodeGenerator();
  ...
  */
  ...
  async compile(request: RealtimeCompilationRequest): Promise<RealtimeCompilationResult> {
    ...
    try {
      ...
      const tokens   = this.lexer.tokenize(request.sourceCode);
      ...
      const analysisResult = this.analyzer.analyze(ast, symbolTable);
      const analysisTime  = performance.now() - analysisStart;

      /* collect errors and warnings from analysis */
      if  (analysisResult.errors) {
        result.errors.push(...this.convertErrors(analysisResult.errors));
      }

      if  (analysisResult.warnings) {
        result.warnings.push(...this.convertWarnings(analysisResult.warnings));
      }
      ...
      if  (result.errors.length === 0) {
        ...
        for  (const target of request.targets) {
          ...
          if  (generatedCode.success) {
            ...
            const outputPath = this.generateOutputPath(request.filePath, target);

            const generatedFile: GeneratedFile = {
              path:    outputPath,
              content: generatedCode.generatedCode || '',
              target:  target,
              size:    Buffer.from(generatedCode.generatedCode || '').length
            };

            if  (request.includeSourceMaps && generatedCode.sourceMap) {
            ...
            if  (generatedCode.errors) {
              result.errors.push(...this.convertCodegenErrors(generatedCode.errors));
            }
      ...
      result.performance = {
        ...
        linesPerSecond: this.calculateLinesPerSecond(request.sourceCode, totalTime)
      };
  ...
  */
  ...
  private mapErrorCategory(type: string): ErrorCategory {
    ...
    switch (type) {
      case 'syntax':    return ErrorCategory.SYNTAX_ERROR;
      case 'type':      return ErrorCategory.TYPE_ERROR;
      case 'semantic':  return ErrorCategory.SEMANTIC_ERROR;
      case 'runtime':   return ErrorCategory.RUNTIME_ERROR;
      default:          return ErrorCategory.COMPILATION_ERROR;
    }
  ...
  private mapWarningCategory(type: string): WarningCategory {
    ...
    switch (type) {
      case 'performance':   return WarningCategory.PERFORMANCE;
      case 'deprecated':    return WarningCategory.DEPRECATED;
      case 'unused':        return WarningCategory.UNUSED;
      case 'style':         return WarningCategory.STYLE;
      case 'compatibility': return WarningCategory.COMPATIBILITY;
      case 'security':      return WarningCategory.SECURITY;
      default:              return WarningCategory.STYLE;
    }

  }

  private generateOutputPath(sourcePath: string, target: CompilationTarget): string {
...
export class CompilationCache {
  ...
  constructor(maxSize: number = 100, maxAge: number = 3600000) { /* 1 hour */
