...
class TestRunner {
  ...
  */
  ...
  addTest(name: string, description: string, testFn: () => Promise<void>): void {
  ...
  */
  ...
  async run(): Promise<TestSuiteResult> {
    ...
    console.log('\n🚀 Starting WORLDSRC Tooling Test Suite...\n');
...
class TestEnvironment {
  ...
  */
  ...
  async createTooling(config?: Partial<ToolingConfiguration>): Promise<WorldSrcTooling> {
  ...
  */
  ...
  runner.addTest(
    ...
    async () => {
      ...
      const response = await adapter['session'].handleRequest(breakpointRequest);
  ...
  */
  ...
  runner.addTest(
    ...
    async () => {
      ...
      const vscodeConfig = await ideManager.generateEditorConfig('vscode' as any);
  ...
  */
  ...
  runner.addTest(
    ...
    async () => {
      ...
      const largeSource = Array(1000).fill(0).map((_, i) =>
        `function test${i}(): void { console.log("test ${i}"); }`
      ).join('\n');
