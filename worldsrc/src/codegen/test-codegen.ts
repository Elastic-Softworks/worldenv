...
async function testTypeScriptGeneration(): Promise<void> {
  ...
  try {
    ...
    console.log(`  ✗ TypeScript generation exception: ${error}`);
...
async function testAssemblyScriptGeneration(): Promise<void> {
  ...
  try {
    ...
    if  (result.success) {
      console.log('  ✓ AssemblyScript generation successful');
      console.log(`  Generated code length: ${result.code.length} characters`);
    } else {
      console.log('  ✗ AssemblyScript generation failed');
      console.log('  Errors:', result.errors);
    }

  } catch (error) {

    console.log(`  ✗ AssemblyScript generation exception: ${error}`);
  ...
  try {
    ...
    if  (result.success) {
      ...
      for (const [target, targetResult] of result.results) {
        console.log(`    ${target}: ${targetResult.success ? 'success' : 'failed'} (${targetResult.files.length} files)`);
      }
    } else {
      console.log('  ✗ Multi-target compilation failed');
      console.log('  Errors:', result.errors);
    }

  } catch (error) {

    console.log(`  ✗ Multi-target compilation exception: ${error}`);
...
async function testSourceValidation(): Promise<void> {
  ...
  try {
    ...
    /* test valid code */
    const validResult = await generator.validateSource(SAMPLE_WORLDSRC_CODE, 'valid.wsrc');
    console.log(`  Valid code: ${validResult.valid ? '✓' : '✗'} (${validResult.errors.length} errors, ${validResult.warnings.length} warnings)`);
    ...
    const invalidResult = await generator.validateSource(invalidCode, 'invalid.wsrc');
    console.log(`  Invalid code: ${!invalidResult.valid ? '✓' : '✗'} (${invalidResult.errors.length} errors expected)`);

  } catch (error) {

    console.log(`  ✗ Source validation exception: ${error}`);
...
async function testOutputManagement(): Promise<void> {
  ...
  try {
    ...
    if  (result.success) {
      console.log('  ✓ Output management successful');
      ...
      console.log('  ✗ Output management failed');
      console.log('  Errors:', result.errors);
    }

  } catch (error) {

    console.log(`  ✗ Output management exception: ${error}`);
...
async function testErrorHandling(): Promise<void> {
  ...
  try {
    ...
    console.log(`  Empty source: ${!emptyResult.success ? '✓' : '✗'} (error handling worked)`);
    ...
    console.log(`  Unsupported features: handled with ${unsupportedResult.warnings.length} warnings`);
...
export async function demonstrateUtilities(): Promise<void> {
  ...
  try {
    ...
    const isValid = await CodeGenerationUtilities.validateString(SAMPLE_WORLDSRC_CODE);
