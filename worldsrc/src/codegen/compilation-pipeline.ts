...
/*
         ...
         compilation-pipeline.ts
           ---
           compilation pipeline manager for WORLDSRC language.
...
export interface PipelineEvents {
  ...
  onError?(error: Error, stage: PipelineStage, target?: CompilationTarget): void;
...
export class CompilationPipeline {
  ...
  */
  ...
  private initializeGenerators(): void {
    this.generators.set(CompilationTarget.TYPESCRIPT, new TypeScriptGenerator());
    this.generators.set(CompilationTarget.ASSEMBLYSCRIPT, new AssemblyScriptGenerator());
    ...
    try {
      ...
      for (const target of targets) {
        if (!this.generators.has(target)) {
          this.addError(result, `Unsupported compilation target: ${target}`, 'UNSUPPORTED_TARGET');
          return result;
        }
      }
      ...
      if (tokens.length === 0) {
        this.addError(result, 'No tokens generated from source code', 'EMPTY_TOKENS');
        return result;
      }
      ...
      if (!ast) {
        this.addError(result, 'Failed to generate AST from tokens', 'PARSING_FAILED');
        return result;
      }
      ...
      for (const target of targets) {
        try {
          ...
          const generationResult = await generator.generate(ast as Program, targetOptions);
          ...
          this.reportError(error as Error, PipelineStage.CODE_GENERATION, target);
        }
  ...
  */
  ...
  private reportStageStart(stage: PipelineStage, target?: CompilationTarget): void {
    if (this.events.onStageStart) {
      this.events.onStageStart(stage, target);
    }
  }

  private reportStageComplete(stage: PipelineStage, target?: CompilationTarget): void {
    if (this.events.onStageComplete) {
      this.events.onStageComplete(stage, target);
    }
  }

  private reportError(error: Error, stage: PipelineStage, target?: CompilationTarget): void {
  ...
  private addError(result: CompilationResult, message: string, code: string): void {
  ...
  private addWarning(result: CompilationResult, message: string, code: string): void {
  ...
  */
  ...
  private processGenerationResult(
    ...
    const filename = this.generateOutputFilename(target, result.request.filename);
  ...
  private generateOutputFilename(target: CompilationTarget, sourceFilename?: string): string {
    const baseName = sourceFilename ? sourceFilename.replace(/\.[^.]+$/, '') : 'output';
  ...
  private generatePackageJson(result: CompilationResult): string {
    ...
    const packageConfig: any = {
      name: (metadata as any).projectName || 'worldsrc-project',
      version: (metadata as any).version || '1.0.0',
      description: (metadata as any).description || 'Generated from WORLDSRC',
      author: (metadata as any).author || 'WORLDSRC Compiler',
      main: 'index.js',
      types: 'index.d.ts',
      scripts: {},
      dependencies: {},
    ...
    if (result.outputFiles.has(CompilationTarget.ASSEMBLYSCRIPT)) {
      ...
      packageConfig.scripts['build:as'] = 'asc assembly/index.ts -b build/optimized.wasm -O3z';
    }
  ...
  private generateReadme(result: CompilationResult): string {
    ...
    let readme = `# ${(metadata as any).projectName || 'WORLDSRC Project'}\n\n`;
    readme += `${(metadata as any).description || 'Generated from WORLDSRC language'}\n\n`;
    ...
    readme += '## Usage\n\n';
    readme += 'This project was generated from WORLDSRC source code. ';
    readme += 'Refer to the WORLDSRC documentation for more information.\n\n';
  ...
  private generateBuildConfig(target: CompilationTarget, result: CompilationResult): string | null {
    switch (target) {
      case CompilationTarget.TYPESCRIPT:
  ...
  */
  ...
  public registerGenerator(target: CompilationTarget, generator: CodeGenerator): void {
  ...
  public getGeneratorInfo(target: CompilationTarget): { name: string; version: string } | null {
    const generator = this.generators.get(target);
    return generator ? { name: generator.name, version: generator.version } : null;
  }
}
