 ...
 *
 * Manages recently opened projects list with persistence.
 * Handles validation and cleanup of stale entries.
 */
...
class RecentProjectsManager {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      if (exists) {
        ...
        const data = await fileSystem.readJSON<RecentProject[]>(
          this.storage_path
        );
  ...
  public async addProject(
    ...
    try {
      ...
      const existing_index = this.recent_projects.findIndex(
        p => p.path === project_path
      );
  ...
  public async removeProject(project_path: string): Promise<void> {
    ...
    try {
      ...
      this.recent_projects = this.recent_projects.filter(
        p => p.path !== project_path
      );
    ...
    try {
      ...
      for (const project of this.recent_projects) {
        ...
        if (exists) {
          ...
          const project_file_exists = await fileSystem.exists(
            project_file
          );
  ...
  private async saveToStorage(): Promise<void> {
    ...
    try {
      ...
      await fileSystem.ensureDirectory(
        path.dirname(this.storage_path)
      );
