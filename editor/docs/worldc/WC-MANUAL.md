...
WORLDC is a high-performance game programming language designed for WORLDENV. It combines C-like syntax with C++ features, TypeScript capabilities, and modern language features, creating a powerful superset of C, C++, and TypeScript specifically optimized for game development.
...
edict PI: float = 3.14159f;    // Constant declaration (like const)
pass;                          // No-operation statement
invoke printf("Hello World"); // Explicit function call
...
edict float GRAVITY = 9.81f;    // WORLDC simplified const
...
// WORLDC invoke syntax
invoke add(5, 10);              // Explicit function call
invoke multiply(3.5, 2.0);      // Works with any function
...
// Using invoke syntax
invoke processScore(score);
invoke updateHealth(damage);

// No-op statements for control flow
if (score < 0) {
    pass;  // Do nothing for negative scores
}
...
void setupScene() {
    ...
    // Use WORLDC wrappers for game logic
...
// Mixed with WORLDC 2D functions
void render2DGame() {
...
// Hot reload
jit_hot_reload(jit, "game_logic.wc");
```
...
WORLDC Language Version: 0.2.0 (Pre-Alpha)
Specification Date: October 2025

## References

- WORLDC Lexicon: `WC-LEXICON.md`
