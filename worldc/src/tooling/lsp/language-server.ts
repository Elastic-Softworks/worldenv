...
export class WorldSrcLanguageServer extends EventEmitter {
  ...
  private capabilities: WorldSrcServerCapabilities =
    {} as WorldSrcServerCapabilities;
  ...
  */
  ...
  private async analyzeDocument(uri: string): Promise<void> {
    ...
    try {
      ...
      this.lexer = new Lexer(doc.content, {});
      const tokens = this.lexer.tokenize();

      /* parse into AST */
      this.parser = new Parser(tokens, {});
      doc.ast = this.parser.parse();
  ...
  */
  ...
  private getFunctionCallAtPosition(
    doc: DocumentInfo,
    position: Position
  ): any {
    return null;
