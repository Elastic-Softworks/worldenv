...
/**
 * WORLDC Error Handler
 *
 * Central error handling system for all WORLDC language processing.
 * Handles error reporting, recovery, logging, and user feedback.
 */
export class WorldCErrorHandler {
  private errors: WorldCError[] = [];
  private warnings: WorldCError[] = [];
   ...
   */
  private reportError(error: WorldCError): void {
    /* Check if we've exceeded error limit */
    if (this.errorCount >= this.maxErrors) {
      if (this.errorCount === this.maxErrors) {
        const limitError: WorldCError = {
          type: ErrorType.COMPILATION_ERROR,
          severity: ErrorSeverity.FATAL,
   ...
   */
  private logError(error: WorldCError): void {
   ...
   */
  private formatError(error: WorldCError): string {
    let formatted = '';
   ...
   * Get all warnings
   */
  public getWarnings(): WorldCError[] {
  ...
  public generateReport(): string {
    ...
    report +=
      '================================================================================\n';
    report += 'WORLDC COMPILATION REPORT\n';
    report +=
      '================================================================================\n';
 ...
 */
export const globalErrorHandler = new WorldCErrorHandler();

/**
 * Utility function to create and throw WORLDC errors
 */
export function throwWorldCError(
  type: ErrorType,
  message: string,
  location?: SourceLocation
): never {
